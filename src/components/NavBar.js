...
const NavBar = () => {
  ...
  return (
    <header className='w-full px-32 py-8 font-medium flex items-center justify-between dark: text-light relative z-10 lg:px-16 md:px-12 sm:px-8'>
      ...
      >
        <span
          className={`bg-dark dark:bg-light block transition-all duration-300 ease-out h-0.5 w-6 rounded-sm ${isOpen} ? 'rotate-45 translate-y-1' : 
          '-translate-y-0.5'}`}
        ></span>
        <span
      ...
      <div className='w-full flex justify-between items-center lg:hidden'>
        ...
        <nav className='flex items-center justify-center flex-wrap'>
          ...
          <button
            ...
            className={`ml-[20px] flex items-center justify-center rounded-full p-1 
      ...
      {isOpen ? (
        ...
        >
          ...
          <nav className='flex items-center justify-center flex-wrap mt-2'>
            ...
            <button
              onClick={() => setMode(mode === 'light' ? 'dark' : 'light')}
              className={`ml-[20px] sm:ml-1 xs:ml-1  flex items-center justify-center rounded-full p-1 
          ${mode === 'light' ? 'bg-dark text-light' : 'bg-light text-dark'} `}
            >
              {mode === 'dark' ? (
                <SunIcon className={'fill-dark'} />
